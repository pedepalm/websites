...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === "GET") {
    return {
      ...
      body: JSON.stringify({ ok: true, service: "wxcc-scheduled-callback-proxy" })
    };
  }
  ...
  try {
    const response = await fetch(WXCC_SCHEDULED, {
      method: "POST",
      headers: {
    ...
    return {
      statusCode: 502,
      body: JSON.stringify({ error: "Could not reach the Webex Contact Center scheduled-callback API." })
    };
  }
};
